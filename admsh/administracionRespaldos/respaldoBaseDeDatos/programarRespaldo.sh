        ...
        if [ $dom -ge -1 -a $dom -lt 32 ]; then
            ...
            if [ $moy -ge -1 -a $moy -lt 13 ]; then
                ...
                if [ $dow -ge -1 -a $dow -lt 8 ]; then
                    ...
                    echo "$minuto $hora $dom $moy $dow administracionRespaldos/respaldoBaseDeDatos/generarRespaldo.sh" > "administracionRespaldos/respaldoBaseDeDatos/politicaRespaldo.txt"
                    success=1
                fi
            fi
        fi
...
if [ "$success" = "0" ]; then
    ...
    echo "[X] `date +'%F %T'`: Programar respaldo (base de datos)" >> "`cat configuracion/ubicacionLog.conf`/audit.log"
else
    echo "[O] `date +'%F %T'`: Programar respaldo (base de datos)" >> "`cat configuracion/ubicacionLog.conf`/audit.log"
fi
